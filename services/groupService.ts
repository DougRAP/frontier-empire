...
export const groupService = {
  ...
  async createGroup(name: string): Promise<PlayerGroup> {
    ...
    // Save to local "DB" so existing code keeps working
    const dbLocal = JSON.parse(localStorage.getItem(DB_KEY) || "{}");
    dbLocal[id] = newGroup;
    localStorage.setItem(DB_KEY, JSON.stringify(dbLocal));

    // Update user session
    localStorage.setItem(USER_KEY, id);

    // Seed some mock scores for the new group so it's not empty (still local for now)
    const seedScores: LeaderboardEntry[] = [
      {
        id: "bot1",
        playerName: "Settler One",
